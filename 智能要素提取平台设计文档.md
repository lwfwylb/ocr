...
提取策略用于定义“某类文档的某个字段应该如何提取、如何兜底、如何判定是否可信”。平台应支持模板级策略和字段级策略，字段级策略优先级高于模板级策略。
...
| required | 是否必填 |
| multiple | 是否允许多个值 |
| extractionMethods | 支持的提取方式，如 LLM、REGEX、TABLE、KEYWORD |
| prompt | AI 提取提示词 |
| jsonSchema | AI 输出 JSON Schema |
| regexPattern | 正则表达式 |
...
| MULTI_EXTRACTOR_VOTE | 高风险关键字段 | 多种方式同时提取，结果一致则通过，不一致进入复核 |
...
```json
{
  ...
  "fieldConfig": {
    ...
    "dataType": "amount",
    "required": true,
    "prompt": "请提取付款金额，优先提取阿拉伯数字金额；如果只有大写金额，请转换为数字金额。",
    "jsonSchema": {
      "type": "object",
      "properties": {
        "amount": {
...
| multiple | 是否多值 |
| sort_no | 排序 |
| extract_methods | AI、REGEX、TABLE、KEYWORD |
| strategy_config | 策略配置 JSON |
| prompt_hint | 字段提示词补充说明 |
| json_schema | 字段 JSON Schema |
