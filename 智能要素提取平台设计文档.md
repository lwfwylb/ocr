...
| regexGroup | 使用第几个捕获分组作为字段值 |
| regexFlags | 正则标记，如 i、m |
| strategy | 策略链，如 AI 优先正则兜底、正则优先 AI 兜底、AI 与正则比对 |

交互要求：

- 左侧展示字段列表，用于快速定位字段级正则规则；字段列表标记“AI统一”和“正则”。
- 右侧使用 Tabs 分为“AI 统一提示词”“字段级正则”“策略预览”。
- “AI 统一提示词”展示配置级启用开关、系统提示词、用户提示词、覆盖字段清单和输出 JSON 要求。
- “字段级正则”只维护当前选中字段的正则规则和测试样例。
- 正则规则需要支持输入测试文本并立即预览匹配结果。
- AI 提示词应由字段配置和落库映射自动生成，允许高级用户编辑全局提示词，但不提供字段级 AI 提示词。
- 当 AI 与正则提取结果不一致时，可按策略进入人工复核。
